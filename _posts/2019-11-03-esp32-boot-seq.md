...
```
- after 2nd stage bootloder gets done printing some useful information, PRO-CPU prints some other info and trasnfers the control over to the the APP-CPU
...
```
- APP-CPU starts executing the Application binary machine instructions & Starts the FreeRtos Scheduler thus our application begins running
...
- During operation / normal runtime If any of the reset reason triggers then all the sequence happens again. 

- If The First stage bootloder does not get any data over UART within a timeout then it jumps to the 2nd stage bootloader and all the subsequent steps occur as explained above.


By embedding the 1st stage bootloader in esp32 mcu , espressif has made programming of the module very easy. This is one of the reason why esp32 module does not get bricked that much and the choice of the UART interface makes the experience much more easy compared to other mcu vendor ecosystems.
